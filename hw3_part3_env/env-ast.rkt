...
(define make-thunk
  (lambda (var env) (list 'thunk var env)))
(define thunk?
           ...
           (cond ((eq? (car x) 'L4)
                 ...
                 ((eq? (car x) 'if)
                  (make-if-exp (parseL4 (second x))
                               ...
                               (parseL4 (fourth x))))
                 ((eq? (car x) 'lambda)
                  (make-proc-exp (map (lambda(y)
                                        (if (and (list? y)  (eq? (length y) 2) (eq? (second y) 'lazy))
                                                     (make-var-lazy-decl (car y))
                                                     (make-var-decl y))) (second x))
                                 (map parseL4 (cddr x))))
                 ((eq? (car x) 'let)
