...
(define env-box-eval 
  (lambda (exp env)
    (cond ((num-exp? exp) (num-exp->val exp))
          ...
          ((prim-op? exp) exp)
          ((thunk? exp) (env-box-eval (thunk->var exp) env))
          ...
          ((app-exp? exp)
           ...
           (let ((procedure  (env-box-eval (app-exp->rator exp) env)))
             (if (prim-op? (app-exp->rator exp))
               (env-box-apply-procedure procedure
                               (map (lambda (rand) (env-box-eval rand env))
                                    (app-exp->rands exp)))
                               
                 (env-box-apply-procedure procedure (map (lambda(operand variable) (if (var-lazy-decl? variable)
                                                                                  (make-thunk operand env)
                                                                                  (env-box-eval operand env)))
                                                     (app-exp->rands exp) (closure->params procedure)))
                 )))          
