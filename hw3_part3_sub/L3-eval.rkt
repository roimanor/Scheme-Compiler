  ...
  (lambda (exp env)
    (cond ((num-exp? exp) (num-exp->val exp))
          ...
          ((app-exp? exp)
           ...
           (let ((procedure  (L3-applicative-eval (app-exp->rator exp) env)))
             (if (prim-op? (app-exp->rator exp))
               (L3-apply-procedure procedure
                               (map (lambda (rand) (L3-applicative-eval rand env))
                                    (app-exp->rands exp))
                               env)
                 (L3-apply-procedure procedure (map (lambda(operand variable) (if (var-lazy-decl? variable)
                                                                                  operand
                                                                                  (L3-applicative-eval operand env)))
                                                     (app-exp->rands exp) (closure->params procedure)) env)
                 )))                   
    ...
    (cond ((prim-op? proc)
          ...
          ((closure? proc)
           (let ((vars (map var-decl->var (closure->params proc)))
             ...
             (let ((lit-args (map L3-value->lit-exp args)))
  ...
  (letrec ((replace
            (lambda (exp)
              (cond ((num-exp? exp) exp)
                    ...
                    ((proc-exp? exp)
                     (let ((tags (map var-lazy-decl->tag (proc-exp->params exp)))   
                           (old-params (map var-decl->var (proc-exp->params exp)))
                           (body (proc-exp->body exp)))
                       (let ((new-param-names (map gen-var old-params))
                             ;; Recurse on body
                             (renamed-body (map replace body))) 
                         (make-proc-exp (map make-var-decl-tag new-param-names tags) 
