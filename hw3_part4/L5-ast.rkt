...
;; <composite-te> ::= <proc-te> | <tuple-te> | <pair-te>
...
(define make-pair-te
  (lambda (type1 type2)
    (list 'pair-te type1 type2)))
(define pair-te?
  (lambda (x) (and (list? x) (= (length x) 3) (eq? (car x) 'pair-te))))
(define pair-te->type1
  (lambda (pair-te) (second pair-te)))
(define pair-te->type2
  (lambda (pair-te) (third pair-te)))
;; Purpose: collect all the components in a composite te
;; Signature: proc-te->components(proc-te)
;; Type: [Proc-te -> List(Texp)]
(define pair-te->components
  (lambda (pair-te)
    (cons (pair-te->type1 pair-te)
          (pair-te->type2 pair-te))))
...
;; pair-exp
;; Type: [cexp * cexp -> pair-exp]
(define make-pair-exp
  (lambda (var1 var2) (list 'pair-exp var1 var2)))
(define pair-exp?
  (lambda (x) (and (list? x) (= (length x) 3) (eq? (car x) 'pair-exp))))
(define pair-exp->var1
  (lambda (pair-exp) (second pair-exp)))
(define pair-exp->var2
  (lambda (pair-exp) (third pair-exp)))
  ...
  (lambda (texp)
    (cond ((eq? texp 'number) (make-num-te))
          ...
          ((list? texp)
           ...
           (if (or (eq? (car texp) 'Pair) (eq? (car texp) 'pair))
               (make-pair-te (parse-texp (cadr texp)))
  ...
  (lambda (te)
    (letrec ((unparse-tuple (lambda (param-tes)
      ...
      (cond ((eq? te 'num-te) 'number)
            ...
            ((eq? te 'void-te) 'void)
            ((eq? te 'pair-te) (list 'pair unparse-texp(pair-te->type1 te) unparse-texp(pair-te->type2 te)))
            ((tvar? te)
  ...
  (lambda (te1 te2 succ fail)
    (cond ((or (empty? te1) (empty? te2))
          ...
          ;((and (pair-te? te1) (pair-te? te2))
           ;(match-tvars-in-te$  (list (pair-te->t1 te1) (pair-te->t2 te1)) (list (pair-te->t1 te2) (pair-te->t2 te2)) succ fail))
