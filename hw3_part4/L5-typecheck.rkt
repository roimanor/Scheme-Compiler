...
(define typeof-def-exp
  (lambda (def tenv)
    (check-equal-type! (var-decl->texp (def-exp->var def))
                       (typeof-exp (def-exp->val def) tenv)
                       def)
   (make-void-te)
))
...
(define typeof-prim-op
  (lambda (x)
    (let ((num-op-te      (parse-texp '(number * number -> number)))
          ...
          (pair-op-te     (parse-texp '(type1 * type2 -> (pair type1 type2))))
          (pair-car-te    (parse-texp '((pair type1 type2) -> type1)))
          (pair-cdr-te    (parse-texp '((pair type1 type2) -> type2)))
          (x (prim-op->op x)))
      (cond ((eq? x '+) num-op-te)
            ((eq? x '-) num-op-te)
            ((eq? x '*) num-op-te)
            ...
            ((eq? x '=) num-comp-op-te)
            ((eq? x 'not) bool-op-te)
            ((eq? x 'cons) pair-op-te)
            ((eq? x 'car) pair-car-te)
            ((eq? x 'cdr) pair-cdr-te)
          ))))
